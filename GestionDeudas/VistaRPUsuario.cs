...
{
    public partial class VistaRPUsuario : Form
    {
        ...
        public VistaRPUsuario(ObjUsuario objUsuario)
        {
            usuario = objUsuario;
            InitializeComponent();
            this.txtCed.KeyPress += new KeyPressEventHandler(txtCed_KeyPress);
        ...
        private void txtMotMin_TextChanged(object sender, EventArgs e)
        {
            if (txtCed.Text == "")
            {
                reportes.CargarTodosLosMovimientosPorUsuarioEnDGV(dgv);
                ColorearFilasPorTipo(dgv);
            }
            else { 
                reportes.CargarMovimientosPorUsuarioFiltradoEnDGV(dgv,txtCed.Text);
                ColorearFilasPorTipo(dgv);
            }
        }
        ...
        private void VistaRPUsuario_Load(object sender, EventArgs e)
        {
            reportes.CargarTodosLosMovimientosPorUsuarioEnDGV(dgv);
            EstilizarDGV(dgv);
            ColorearFilasPorTipo(dgv);
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                EnviarMail.EnviarCorreoConPDFEnMemoria(usuario.correo, pdfBytes, "Reporte de sistema", "Reportes por usuario");
