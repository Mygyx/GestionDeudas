...
{
    public partial class VistaRPMonto : Form
    {
        Reportes reportes = new Reportes();
        ExportarExcel excel = new ExportarExcel();
        ObjUsuario usuario;

        public VistaRPMonto(ObjUsuario objUsuario)

        {
            usuario = objUsuario;
            InitializeComponent();
            this.txtMotMin.KeyPress += new KeyPressEventHandler(txtMotMin_KeyPress);
            this.txtMotMax.KeyPress += new KeyPressEventHandler(txtMotMax_KeyPress);
        ...
        private void VistaRPMonto_Load(object sender, EventArgs e)
        {
            reportes.CargarMovimientosPorMontoEnDGV(dgv, 0, decimal.MaxValue);
            EstilizarDGV(dgv);
            ColorearFilasPorTipo(dgv);
        }

        private void txtMotMin_TextChanged(object sender, EventArgs e)
        {
            try
            {
                decimal montoMin = 0;
                ...
                bool minVacio = string.IsNullOrWhiteSpace(txtMotMin.Text);
                bool maxVacio = string.IsNullOrWhiteSpace(txtMotMax.Text);

                // Si mínimo tiene valor, lo convertimos
                if (!minVacio)
                    montoMin = Convert.ToDecimal(txtMotMin.Text);

                // Si máximo tiene valor, lo convertimos
                if (!maxVacio)
                    montoMax = Convert.ToDecimal(txtMotMax.Text);

                // Ejecutar solo una vez con los valores calculados
                reportes.CargarMovimientosPorMontoEnDGV(dgv, montoMin, montoMax);
                ColorearFilasPorTipo(dgv);
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error: el monto ingresado no es válido.\n" + ex.Message, "Monto inválido", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }


        }

        private void txtMotMax_TextChanged(object sender, EventArgs e)
        {
            try
            {
                decimal montoMin = 0;
        ...
        private void txtMotMin_KeyPress(object sender, KeyPressEventArgs e)
        {
            // Acepta solo dígitos, control (como backspace), y un solo punto decimal
        ...
        private void txtMotMax_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (!char.IsControl(e.KeyChar) && !char.IsDigit(e.KeyChar) && e.KeyChar != '.')
            {
                e.Handled = true;
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                EnviarMail.EnviarCorreoConPDFEnMemoria(usuario.correo, pdfBytes, "Reporte de sistema", "Reportes por Monto");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            using (SaveFileDialog sfd = new SaveFileDialog())
            {
                ...
                {
                    excel.ExportarDGVaExcel(dgv, sfd.FileName, usuario.correo);
                }
            }
        }
    }
